...
* [B:](http://asn.desire2learn.com/resources/S2696092) Storing RDF data
    ...
    * [C:] Knows directories, such as Linked Open Vocabularies, for discovering SPARQL endpoints.
    * [C:] Understands that triplestores usually provide a SPARQL endpoint.
    * [C:] Understands the difference between a triplestore (which is analogous to a MYSQL database manager) and an endpoint (which may or may not also be a triplestore).
    * [C:] Is aware of both commercially available and open-source triplestores.
    * [C:] Is aware that a triplestore may or may not support SPARQL or SPARQL-like queries.
    * [C:] Is aware of how triplestores handle a graph name unique in the context of that dataset or an unnamed graph (e.g., explicitly specifying FROM NAMED in your query).
    * [C:] Understands how named graphs can designate specific sets of triples in a triplestore.
    ...
    * [C:] Understands the role of the SPARQL processor working in conjunction with the triplestore or endpoint to process and retrieve data.
    * [C:] Understands using the SERVICE keyword to query remote RDF data by pointing it to a SPARQL endpoint directly, obviating the need to point to an RDF file somewhere (or at a service delivering the equivalent of an RDF file).
    * [C:] Understands how using SERVICE differs from not using SERVICE.
    * [C:] Understands how SERVICE allows you to embed a query within a query.
    * [C:] Understands how the SERVICE keyword allows you to get at otherwise siloed data.
    * [C:] Understands that the SERVICE keyword can pass a graph pattern or query to the SPARQL endpoint.
    * [C:] Understands that adding LIMIT and OFFSET keywords to queries can reduce the scope of queries to SPARQL endpoints.
    * [C:] Understands that a program can query a SPARQL endpoint via HTTP by attaching an escaped version of the query as a URI parameter.
